...
function secondMenuSetup(){
    ...
    curPage.dishAttrOperation = function(event){
     ...
     curPage.matchPrice=function(curDishInfo_chang){
        ...
        var dishAttrOptionPriceDataSpecial;
        if(specialDish){
             dishAttrOptionPriceDataSpecial = specialDish.dishAttrOptionPriceData;
        }
     ...
     /** 
      *   dishAttrOptionPriceData:价格匹配表
      *   allSelAttr:当前所有选中的属性
      *   isNormal:价格匹配表的来源
      */
     curPage.refreshDishPrice=function(curDishInfo_chang,dishAttrOptionPriceData,allSelAttr,isNormal){
         ...
         *  同级属性不禁止，包含于某条价格匹配信息的不禁止，其余禁止，递归本次不禁止属性和上次去交集，遍历结束将其余属性且单选禁止
         */
        if(curDishInfo_chang.groupName =='特价菜'){
    ...
    curPage.setupDisableAttr = function(effectAttrs,curDishInfo_chang){
        /**将不有效的置灰 ，有效且之前不是高亮的置为正常 */
        function operationAttrState (dishAttrData){
              for (var i = 0; i < dishAttrData.length; i++) {
                var obj = dishAttrData[i];
        ...
        operationAttrState(curDishInfo_chang.dishAttrData);
        operationAttrState(curDishInfo_chang.dishAttrDatahasInf);
